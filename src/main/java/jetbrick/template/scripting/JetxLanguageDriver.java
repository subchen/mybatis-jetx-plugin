 ...
 * @author huangx
 */
public class JetxLanguageDriver implements LanguageDriver {
	private JetEngine engine;

	public JetxLanguageDriver() {
		this.engine = JetEngineHolder.getJetEngine();
	}
	...
	protected SqlSource createSqlSource(JetTemplate template, Configuration configuration) {
		return new JetxSqlSource(template, configuration);
	}
	...
	private SqlSource createSqlSource(Configuration configuration, String scriptText) {
		...
		if (scriptText.contains(" ")) {
			template = engine.createTemplate(scriptText);
		} else {
			// 特殊 sql 区分
			if (scriptText.startsWith("&")) {
				scriptText = scriptText.substring(1);
			}
			template = engine.getTemplate(scriptText + engine.getConfig().getTemplateSuffix());
		}
